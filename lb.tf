locals {
  enabled_lb_envs = ["sandbox", "demo", "demo-int", "ithc", "aat", "perftest", "prod"]
...
resource "azurerm_lb" "demo_int" {
  ...
  location            = var.location
  resource_group_name = "ccd-elastic-search-${var.demo_int_rg_name}"

  sku = "Standard"

  frontend_ip_configuration {
    name                          = "LBFE"
    subnet_id                     = data.azurerm_subnet.elastic-subnet-demo-int.id
    private_ip_address_allocation = "Static"
