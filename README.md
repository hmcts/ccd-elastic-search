...
## 7. Monitoring
Each of the individual VMs are monitored by Azure with information about CPU, Availability and Memory usage available via Insights.

An overview of CCD is available in Dynatrace but the CCD Elasticsearch VMs are not yet monitored by this platform. They are however in the XSAIM system via the XDR agents, running on each VM.
...
## 10. Open items / TODOs (to complete with CCD)
